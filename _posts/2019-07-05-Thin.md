...
- 3-D Gradient Correction Loss
  ...
  - $L^G_{GC} = \mathbb{E}[(\nabla_xI^{GT}_{x,y,z}-\nabla_xI^Y_{x,y,z})^2] \\ + \mathbb{E}[(\nabla_yI^{GT}_{x,y,z}-\nabla_yI^Y_{x,y,z})]^2\\ + \mathbb{E}[(\nabla_zI^{GT}_{x,y,z}-\nabla_zI^Y_{x,y,z})^2]$


- Adversarial Loss
  - LSGAN Loss 사용. 
  - $L^D=\frac{1}{2}\mathbb{E}[(D(I^{GT}, I^A, I^S)-1)^2+(D(I^Y, I^A, I^S)-0)^2]$
...
- 3D-Y-Net-GAN Loss : $L_G = L^G_{SC} + \lambda_1L^G_{GC} + \lambda_2L^G_{AD} + \lambda_3L^G_{WR}$
- 3D-DenseU-Net Loss : $L = L_{SC} + \lambda_1L_{GC} + \lambda_3L_{WR}$ 
